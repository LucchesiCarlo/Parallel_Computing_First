...
#pragma omp for schedule(runtime)
            for (int i = 0; i < exp.N; i++) {
                ...
                nextBoids.vx[i] = close_dx * exp.AVOID + (velX - boids.vx[i]) * exp.MATCH + (posX - boids.x[i]) * exp.CENTER + boids
                                  .vx[i];
                nextBoids.vy[i] = close_dy * exp.AVOID + (velY - boids.vy[i]) * exp.MATCH + (posY - boids.y[i]) * exp.CENTER + boids
                                  .vy[i];
